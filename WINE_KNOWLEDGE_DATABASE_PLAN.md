...
- **Wikidata** — Items classified `instance of: wine` (Q282) carry P131 (located in administrative territorial entity) and P17 (country). Confirmed live: 2,993 wine items, 9,265 appellation-to-province rows. Test case confirmed exact: Colli di Scandiano e di Canossa DOC → Province of Reggio Emilia → Italy. Implemented in `lib/wines/region-hierarchy-checker.ts` (commit `0969473`). query.wikidata.org is blocked at this research sandbox's network proxy — proven reachable only from a browser context so far, not yet from this project's own Node/Vercel runtime. That is the one open verification item, not a design gap.
...
- **eAmbrosia identity dataset (Candiago et al. 2022, figshare, CC0)** — real and in use for appellation identity across all 1,177 EU PDOs, but whether "Colli di Scandiano e di Canossa" is literally a row in the file was never directly confirmed — the file (1.2MB) exceeded every available fetch/relay mechanism tried (direct fetch, sandboxed bash, browser navigation, in-page cross-origin fetch, figshare's own article-page preview). Circumstantial case it's included is strong (eAmbrosia is the official EU register, the dataset claims full 1,177-PDO coverage as of Nov 2021, and the DOC is confirmed still active via Emilia-Romagna's own regional government site) — but this is inference, not a row-level check, and should be labeled as such until someone with unrestricted network access opens the file directly.
